...
(function(){
  ...
  function handleUiKeydown(e){
    // La fonction G doit toujours fonctionner pour le debug
    if (e.key === 'g' || e.key === 'G') {
      e.preventDefault();
      const map = window.currentMap || 'unknown';
      const x = window.player ? Math.round(window.player.x) : '?';
      const y = window.player ? Math.round(window.player.y) : '?';
      const msg = `📍 ${map} (${x}, ${y})`;
      
      // Essayer d'afficher le message flottant d'abord
      ...
      console.log(`🎯 Coordonnées: ${map} (${x}, ${y})`);
      return;
    }
    
    // Pour les autres touches, vérifier les conditions
    ...
    switch(k){
      case 'e':
        e.preventDefault();
        // Gestion des interactions avec les banquiers
        if (window.currentMap === "banque" && typeof window.checkBanquierInteractions === 'function') {
          window.checkBanquierInteractions();
        }
        break;
      case 'i':
        e.preventDefault();
