...
function calculateMapCentering() {
    ...
    if (window.currentMap === "mapdonjonslime" || window.currentMap === "mapdonjonslime2" || window.currentMap === "mapdonjonslimeboss") {
        ...
        // Centrage spécial pour la banque - comme les donjons slime
        window.mapOffsetX = Math.max(0, (canvas.width - mapWidth) / 2);
        window.mapOffsetY = Math.max(0, (canvas.height - mapHeight) / 2);
...
async function loadMap(mapName) {
    try {
        ...
        // Initialiser les mentors pour cette map
...
function drawMap() {
    ...
    // Dessiner les mentors
    if (typeof window.MentorManager !== 'undefined' && window.MentorManager.draw) {
        window.MentorManager.draw(ctx);
    }
    
    // Dessiner les banquiers si on est sur la map banque
    if (window.currentMap === "banque" && typeof window.drawBanquiers === "function") {
        window.drawBanquiers(ctx);
    }
